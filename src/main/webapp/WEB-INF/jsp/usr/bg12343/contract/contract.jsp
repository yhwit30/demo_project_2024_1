...
<section class="mt-2 text-xl px-4">
					...
					<c:forEach var="contract" items="${contracts }">
						<tr class="hover">
							...
							<td class="resetBtn" style="display: none;">
								<a href="/usr/bg12343/contract/doContractDelete?contractId=${contract.id }" class="btn btn-sm"
									onclick="if(confirm('${contract.bldgName} 건물의 ${contract.roomNum }호 ${contract.tenantName }과(와)의 계약(${contract.contractStartDate } ~ ${contract.contractEndDate })이 삭제됩니다.\n진행하시겠습니까?') == false) return false;">삭제</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="btns mt-5">
		...
		<button class="btn btn-outline resetBtnShow" onclick="toggleResetBtn();">계약 삭제</button>
		<button class="btn btn-outline resetBtn" onclick="toggleReset();" style="display: none;">계약 삭제 취소</button>
	</div>
